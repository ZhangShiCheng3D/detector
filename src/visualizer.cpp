...
cv::Mat Visualizer::visualize(const cv::Mat& test_image,
    ...
    // Draw ROI boxes (without text)
    if (config_.roi_boxes.enabled) {
        drawROIBoxes(output, rois, result.roi_results);
    }

    // Draw defect boxes only (no text, no arrows, no corner markers)
    if (config_.defect_markers.enabled) {
        for (size_t i = 0; i < rois.size() && i < result.roi_results.size(); ++i) {
            drawDefectBoxesOnly(output, rois[i], result.roi_results[i].defects);
        }
    }

    // Disable all text and info displays
    // - similarity_scores: disabled
    // - legend: disabled
    // - processing_info: disabled
    // - defect_summary: disabled
...
void Visualizer::drawDefectBoxesOnly(cv::Mat& image,
                                      const ROIRegion& roi,
                                      const std::vector<Defect>& defects) {
    cv::Point offset(roi.bounds.x, roi.bounds.y);
    ...
    for (const auto& defect : defects) {
        ...
        if (defect_rect.width <= 0 || defect_rect.height <= 0) continue;
        ...
        // Draw outline only (no corner markers, no arrows, no text)
