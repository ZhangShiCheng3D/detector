...
");
        ...
        private static void RunDemo(ProgramConfig config)
        {
            ...
            using (var detector = new DefectDetectorAPI())
            {
                ...
                int templWidth, templHeight, templChannels;
                detector.GetTemplateSize(out templWidth, out templHeight, out templChannels);
            ...
            List<System.Drawing.Rectangle> selectedROIs, List<float> selectedThresholds)
        {
            ...
            detector.AddROI(200, 420, 820, 320, 0.8f);
            // 生成随机ROI区域
        ...
        private static void ConfigureAlignment(DefectDetectorAPI detector, ProgramConfig config)
        {
            ...
            if (config.AutoAlignEnabled)
            {
                ...
                {
                    case DefectDetectorAPI.AlignmentMode.None:
                        modeStr = "NONE";
                        break;
                    case DefectDetectorAPI.AlignmentMode.FullImage:
                        modeStr = "FULL_IMAGE (slower, whole image transform)";
                        break;
                    case DefectDetectorAPI.AlignmentMode.RoiOnly:
                        modeStr = "ROI_ONLY (faster, recommended)";
                        break;
                };
        ...
        private static void ProcessImages(DefectDetectorAPI detector, List<string> imageFiles, ProgramConfig config)
        {
            ...
            // 限制处理数量
            int processCount = Math.Min(50, imageFiles.Count);
