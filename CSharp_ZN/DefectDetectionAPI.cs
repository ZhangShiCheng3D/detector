...
namespace DefectDetection
{
    ...
    public class DefectDetectorAPI : IDisposable
    {
        ...
        {
            public int Enabled;                       // 是否启用二值图像优化模式
            public int AutoDetectBinary;              // 自动检测输入是否为二值图像
            public int NoiseFilterSize;               // 噪声过滤核大小
            public int EdgeTolerancePixels;           // 边缘容错像素数
            public float EdgeDiffIgnoreRatio;         // 忽略的边缘差异比例
            public int MinSignificantArea;            // 最小显著差异面积
            public float AreaDiffThreshold;           // 区域差异阈值
            public float OverallSimilarityThreshold;  // 总体相似度阈值
            public int EdgeDefectSizeThreshold;       // 边缘区域大瑕疵保留阈值，默认500像素
            public float EdgeDistanceMultiplier;      // 边缘安全距离倍数，默认2.0
            public int BinaryThreshold;               // 二值化阈值，用于提取模板边缘，默认128
        ...
        public struct ROIInfo
        {
            ...
            public int Enabled;                 // 是否启用（注意：当前C++ API硬编码为1）
        ...
        /// </summary>
        /// <remarks>
        /// 注意：C++ DLL返回的是字符串常量，不需要释放。
        /// 如果C++实现改为动态分配，需要添加FreeString调用。
        /// </remarks>
        ...
        public bool SetLocalizationPoints(PointF[] points)
        {
            ...
            if (points.Length < 3)  // 修复：C++需要至少3个点
        ...
        /// <remarks>
        /// 注意：ResultCode为1表示检测正常执行但发现瑕疵，这不是异常。
        /// </remarks>
            ...
            public override string ToString()
            {
                switch (ResultCode)
                {
                    case -1:
                        return "检测执行失败";
                    case 0:
                        return string.Format("检测通过, 无瑕疵, 总耗时:{0:F1}", ProcessingTime);
                    case 1:
